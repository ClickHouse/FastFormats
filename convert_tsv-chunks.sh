...
if [ $# -lt 4 ]; then
    echo -e "\nUsage: $0 OUTPUT_FORMAT SORT INPUT_DIR OUTPUT_DIR [EXTRA_SETTINGS] [TEST_MODE]\n"
    exit 1
fi
...
INPUT_DIR="$3"
OUTPUT_DIR="$4"
EXTRA_SETTINGS="${5:-}"  # Default is empty
TEST_MODE="${6:-false}"  # Default is false
...
echo -e "\n===================================="
echo "  Converting TSV to $OUTPUT_FORMAT"
echo "  Input Directory: $INPUT_DIR"
echo "  Output Directory: $OUTPUT_DIR"
echo "  Sorting Enabled: $SORT"
...
TSV_FILES=($(ls -1 "$INPUT_DIR"/hits_part_*.tsv 2>/dev/null | sort))
...
for tsv_file in "${TSV_FILES[@]}"; do
    ...
    echo "Processing: $tsv_file → $output_file"
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "Error converting: $tsv_file"
    fi
done
